...
class Runner:
    ...
    def insertPpomppuResult(self, stock):
        ...
        ppomppuResult = ppomppu.getTrend(self.PPOMPPU_ID, self.PPOMPPU_PWD, stockName,
                                         lastScrapAt)  # id , password , search
        ...
        for period in self.dbm.getPeriodAll():
            ...
            for stock in self.dbm.getStockList():
                ...
                for filter in filteredTargetList:
                    ...
                    if (countCheck and potentialCheck) and (
                        percentCheck or chanceCheck):  # and filter.get('yesterday') < 0:
    ...
    def getAnalyzeExistData(self, stockName, period):
            ...
            if (total > 0 and resultPrice != 0) and not (plusPercent == 0 or minusPercent == 0):
                if resultPrice < 0:
                    if self.checkDefence(stockName, targetAt):
                        ...
                        minusPoint.append({'name': stockName, 'result': resultPrice, 'point': minusPercent,
                                           'targetAt': str(targetAt)})
                sumPoint.append(
        ...
        pointDict = {
            stockName: {'name': stockName, 'period': period, 'potential': trustPercent, 'total': len(analyzedResult)}}
    ...
    def getAnalyzedTarget(self, itemId, plusChanceIds, point, pointDict, stockName, targetAt, stockCode):
        ...
        return {'name': stockName, stockName: point, 'period': pointDict.get(stockName).get('period'),
                'potential': pointDict.get(stockName).get('potential'),
                'total': pointDict.get(stockName).get('total'), 'targetAt': targetAt.day, 'chance': chanceIds,
                'yesterday': financeResult, 'code': stockCode}
    ...
    def filterPotentialStock(self, periods):
        for period in periods:
            for stock in self.dbm.getAllStockList():
                ...
                if poten.get('count') > self.GUARANTEE_COUNT and poten.get(
                        'potential') < self.FILTER_LIMIT and stock.get('much') == 0:
    ...
    def updateKospiPrice(self):
        ...
        for each in priceInfos:
            ...
            if exist is None:
                self.dbm.insertFinance(stock.get('id'), each.get('date'), high=0, low=0, start=each.get('start'),
                                       final=each.get('final'))
    ...
    def now_progress(self):
        ...
        return 'today percent is : ' + str(forecast_cnt / daily_cnt * 100) + '%'
